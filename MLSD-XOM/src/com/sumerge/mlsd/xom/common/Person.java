...
import com.sumerge.mlsd.xom.common.Appeal.AppealType;
import com.sumerge.mlsd.xom.common.HomeWorkerDetails.WorkType;
...
public class Person {
	...
	public boolean totalApplicantSalaryIsMoreThan(int threshold) {
		...
		if (applicantIncomeDetails != null) {
			for (IncomeDetails incomeDetail : applicantIncomeDetails) {
				if (incomeDetail.getIncomeType() == IncomeType.SALARY) {
	 ...
	 * 
	 * @param appealType
	 *            : The appeal type to check the validity of.
	 * @return if a specific appeal is expired by comparing the todays date with
	 *         the appeal expiration date.
	 */
	public boolean appealISExpired(AppealType appealType) {
		String sourceMethod = "appealISExpired";
		logger.entering(personClassName, sourceMethod);
		List<Appeal> appealsList = this.getAppeals();
